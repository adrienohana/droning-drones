...
from note_lists import amazing_grace_scale_acc

#times = [note[1] for note in amazing_grace_scale_acc]
#accelerations = [note[0] for note in amazing_grace_scale_acc]


times = [1, 0.5, 0.5, 0.5]
accelerations = [0, 0.5, -0.5, 0.5]

control_time = 1/500
control_times, control_velocities = compute_velocity_commands(times, accelerations, control_time)
...
if __name__ == '__main__':
    ...
    lg_stab.add_variable("ctrltarget.vz", "float")
    ...
    with SyncCrazyflie(URI, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        with MotionCommander(scf) as mc:
            ...
            for id,v in enumerate(control_velocities):
                print(v)
                mc.start_up(velocity=v)
                time.sleep(control_time)
            mc.stop()
            # We land when the MotionCommander goes out of scope """


    with open('logdata_1over500.pkl', 'wb') as handle:
