...
if __name__ == "__main__":
    ...
    #NOTE LENGTH !!
    note_length = 0.15
    #PICK CHORDS AND STARTING HEIGHT
    cmds1 = [51585.97752737439, 50479.70332931376, 49235.67746163059, 47842.66978969166, 46301.62390154745, 44616.222088218805,
            42785.777708419184,40832.2954123262, 36621.84170754111, 34418.44165022816, 32192.12052852007, 29973.640918757486, 
            27794.280936225314, 25670.17803451757,23633.878321364187,21690.476247320054]

    cmds2 = [21616.654165416538, 23556.639163916392, 25592.25722572257, 27714.400440044006, 29895.745074507457, 32108.967396739674,
            34335.85158515852, 36535.41204120413, 40752.37473747374, 42715.129512951295, 44550.37353735374, 46244.44494449444,
            47788.23582358237, 49186.30013001301, 50434.08390839083, 51545.2490249025]

    starting_height = 3

    yaw_rates = np.linspace(0,320,16)
        ...
        if i%CTRL_EVERY_N_STEPS == 0:
            ...
            if ctrl.controller_type == POSITION_CONTROL:
                ...
                if l >= len(cmds1):
                    ...
                    target_rpy_rates = yaw_rates[l]
                    action[str(0)], _, _ = ctrl.computeControlFromState(control_timestep=CTRL_EVERY_N_STEPS*env.TIMESTEP,
                                                                    state=obs[str(0)]["state"],
                                                                    target_pos = np.array([0,0,starting_height]),
                                                                    target_rpy_rates=target_rpy_rates
                                                                    )
                    if time.time()-note_start >=note_length:
                        l+=1
                        note_start=time.time()
                #### Compute control for the current way point ############
                rpm, pos_e, delta_rpy = ctrl.computeControlFromState(control_timestep=CTRL_EVERY_N_STEPS*env.TIMESTEP,
                                                                        state=obs[str(0)]["state"],
