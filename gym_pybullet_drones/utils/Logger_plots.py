...
class Logger(object):
    ...
    def midterm_plot(self, filename='test_plot', pwm=False):
        """Logs entries for a single simulation step, of a single drone.
        ...
        row = 0
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, self.states[j, 12, :], label=int(self.states[j, 12, :][len(t)-1]))
        axs[row, col].set_xlabel('time')
        if pwm:
            axs[row, col].set_ylabel('PWM0')
        else:
            axs[row, col].set_ylabel('RPM0')
        axs[row,col].set_xlim(left=self.LOGGING_FREQ_HZ)
        axs[row,col].set_xlim(left=left_limit)
        axs[row, col].set_ylim(lower_limit, upper_limit)

        row = 1
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, self.states[j, 13, :], label=int(self.states[j, 13, :][len(t)-1]))
        axs[row, col].set_xlabel('time')
        if pwm:
            axs[row, col].set_ylabel('PWM1')
        else:
        ...
        row = 2
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, self.states[j, 14, :], label=int(self.states[j, 14, :][len(t)-1]))
        axs[row, col].set_xlabel('time')
        if pwm:
            axs[row, col].set_ylabel('PWM2')
        else:
        ...
        row = 3
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, self.states[j, 15, :], label=int(self.states[j, 15, :][len(t)-1]))
        axs[row, col].set_xlabel('time')
        if pwm:
            axs[row, col].set_ylabel('PWM3')
        else:
        ...
        row = 0
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, np.degrees(self.states[j, 6, :]))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('Roll (deg)')

        row = 1
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, np.degrees(self.states[j, 7, :]))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('Pitch (deg)')

        row = 2
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, np.degrees(self.states[j, 8, :]))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('Yaw (deg)')

        row = 3
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, np.degrees(self.states[j, 11, :]))
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('Wz Yaw Rate (deg/s)')
        ...
        row = 0
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, self.states[j, 3, :])
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('Vx (m/s)')
        row = 1
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, self.states[j, 4, :])
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('Vy (m/s)')
        row = 2
        for j in range(self.NUM_DRONES):
            axs[row, col].plot(t, self.states[j, 5, :])
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('vz (m/s)')
    ...
    def scale_plot(self, filename='test_plot.png', pwm=False):
        """Logs entries for a single simulation step, of a single drone.
        ...
        col = 0
        for j in range(self.NUM_DRONES):
            axs[col].plot(t, self.states[j, 12, :], label="last value : "+str(int(self.states[j, 12, :][len(t)-1])))
        axs[col].set_xlabel('time')
        if pwm:
            axs[col].set_ylabel('PWM0')
        else:
            axs[col].set_ylabel('RPM0')
        axs[col].set_ylim(lower_limit, upper_limit)

        col = 1
        for j in range(self.NUM_DRONES):
            axs[col].plot(t, self.states[j, 2, :], label="last value : "+str(round(self.states[j, 2, :][len(t)-1], 3)))
        axs[col].set_xlabel('time')
        axs[col].set_ylabel('z (m)')
        print(self.states[j, 2, :][len(t)-1])
